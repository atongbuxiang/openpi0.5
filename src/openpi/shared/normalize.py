...
class NormStats:
    ...
    def __post_init__(self):
        ...
        if self.q01 is not None:
            self.q01 = np.asarray(self.q01, dtype=np.float32)
        if self.q99 is not None:
            self.q99 = np.asarray(self.q99, dtype=np.float32)

    @pydantic.field_serializer("mean", "std", "q01", "q99")
    def _serialize_to_list(self, v, _info):
