    ...
    if isinstance(repo_id, list):
        ...
        if data_config.prompt_from_task:
            ...
            for rid in repo_id:
                ...
                # Prefix each task key with the repo_id to ensure uniqueness
                for task_key, task_value in meta.tasks.items():
                    all_tasks[f"{rid}_{task_key}"] = task_value
            print("Debug: all_tasks =", all_tasks)

            # 获取 repo_ids
            repo_ids = dataset.repo_ids
            print("Debug: repo_ids =", repo_ids)
            prompts = _transforms.PromptFromMultiLeRobotTask(all_tasks, repo_ids=repo_ids)
            dataset = TransformedDataset(dataset, [prompts])
